...
Most microbiome-IBD analysis is cross-sectional. It looks for taxa that correlate with disease activity right now. That is a different, easier question than the clinically useful one: whether a patient's current gut microbiome says anything about where their disease is heading in the next few weeks. This project goes after the harder question directly. Given microbiome composition and clinical score at timepoint t, it forecasts the clinical score 2-4 weeks later.

Two problems that a naive version of this analysis gets wrong, and that this project is built around instead:

- **Repeated-measures leakage.** HMP2 subjects have up to 24 timepoints each. A random train/test split lets a patient's near-identical microbiome from adjacent weeks land in both folds. That inflates accuracy on a metric that is not actually measuring generalization to a new patient. Every model here uses subject-grouped cross-validation instead, `GroupKFold` or `LeaveOneGroupOut` on `Participant ID`, never a random split.
- **Uncredited baselines.** A model that combines microbiome plus current score can beat "predict no change" purely by learning a slope and intercept on the current score, with the microbiome contributing nothing. That is exactly what happened during development here (see Validation/Results below). Rather than report the flattering comparison and move on, this got caught by adding a fitted score-only baseline and auditing the model's coefficients with SHAP.
...
**Same-timepoint baseline** (`train_baseline.py`, subject-grouped nested CV, `GroupKFold(5)`/`GroupKFold(4)`): cross-sectional microbiome composition alone turns out to be a weak predictor of same-day clinical activity score. CD mean R² is about -0.09 and UC mean R² is about -0.06, both consistent with zero or worse within one standard deviation. This is reported as it came out, not tuned until it looked better. It is a known-hard task in the field, and it de-risked the modeling approach before the temporal element was added.
...
The source dataset and its original analysis are [Lloyd-Price et al. 2019, *Nature*](https://www.nature.com/articles/s41586-019-1237-9) (the HMP2/IBDMDB paper) and its companion repo, [biobakery/hmp2_analysis](https://github.com/biobakery/hmp2_analysis). That paper's own dysbiosis-scoring approach, median Bray-Curtis dissimilarity to a non-IBD reference cohort, is what `ecology.py`'s `dysbiosis_score` reimplements from scratch, since per-sample scores are not included in the raw downloads and the analysis repo was not pulled in as a dependency. The original paper is a cross-sectional dysbiosis characterization, not a forecasting study. The leave-one-subject-out (t, t+1) forecasting task here, and specifically the score-only baseline and SHAP audit built to check whether "combined beats persistence" claims hold up, are this project's own additions rather than reproductions of established methodology. Most of the modeling itself, ElasticNet with nested subject-grouped CV and arcsin-sqrt-transformed relative abundance, follows standard, well-established microbiome ML practice. It does not invent anything new there.
